            ...
            if (r == true) {
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        successMessage.classList.add("alert", "alert-success", "alert-dismissible", "fade", "show", "mt-3");
                        successMessage.setAttribute("role", "alert");
                        successMessage.innerHTML = "Email is sent";
                        
                        // Append the success message to the container
                        row.parentElement.parentElement.parentElement.parentElement.appendChild(successMessage);
  ...
  if (localStorage.getItem("cart_retrieved") !== "true"){
    $.ajax({
         ...
         success: function(res) {
              ...
              localStorage.setItem("cart", res.length ? res : "[]");  // Ensure the cart is set to an empty array if the response is empty
          }
    });
  }
}
