...
def chinese(request):
    if request.user.is_authenticated:
        return render(request, "orders/chinese.html", context = {"dishes":Chinese.objects.all})
    else:
        return redirect("orders:login")
...
def snack(request):
    if request.user.is_authenticated:
        return render(request, "orders/snack.html", context = {"dishes":Snack.objects.all})
    else:
        return redirect("orders:login")
...
def beverage(request):
    if request.user.is_authenticated:
        return render(request, "orders/beverage.html", context = {"dishes":Beverage.objects.all})
    else:
        return redirect("orders:login")
...
def mark_order_as_delivered(request):
    if request.method == 'POST':
        ...
        recipient_list = [f"{order.username}"]  # Adjust domain as needed
        send_email_to_client(recipient_list)
...
def retrieve_saved_cart(request):
    try:
        ...
        return HttpResponse('[]')  # Return an empty list as a JSON string if no saved cart is found
...
def send_email(request):
    if request.method == 'GET':
        ...
        if order_id:
            ...
            recipient_list = [f"{order.username}"]
            send_email_to_client(recipient_list)
    messages.success(request, 'Email is sent')
